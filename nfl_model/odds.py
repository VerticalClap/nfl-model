...
def extract_consensus_moneylines(odds_raw: list) -> pd.DataFrame:
    """Median moneylines across books; convert to fair win% by removing vig."""
    rows = []
...
def add_implied_probs(df: pd.DataFrame) -> pd.DataFrame:
    """Add raw implied probs and fair (de-vigged) probs. Safe if MLs are missing."""
    out = df.copy()
    if "home_ml" in out:
    ...
    if "home_prob_raw" in out and "away_prob_raw" in out:
        # Build a 2-column Series per row, then assign BOTH columns at once (no KeyError).
        fair = out.apply(
